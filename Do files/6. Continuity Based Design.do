	...
	*-----------------------------------------------------------------------------------------------------------------------------------*
	...
	set more off
		...
		foreach groupvar in 1 2 3 4 5 6 { //
			   ...
			   *if `groupvar' == 1 estout * using "$tables\.xls" 		,  keep(D`cohort')   label  mgroups("Economically active" 	"Paid work" 		"Formal paid work"		,  pattern(1 0 0 1 0 0 1 0 0))  stats(pwolf1 N,  labels("Romano-Wolf"))  cells(b(star fmt(2) ) se(par(`"="("' `")""')  fmt(2)) p(fmt(2))) starlevels(* 0.10 ** 0.05 *** 0.01) replace
			   *if `groupvar' == 2 estout * using "$tables\.xls" 		,  keep(D`cohort')   label  mgroups("Informal paid work" 	"Attending school" 	"Only attending school"	,  pattern(1 0 0 1 0 0 1 0 0))  stats(pwolf1 N,  labels("Romano-Wolf"))  cells(b(star fmt(2) ) se(par(`"="("' `")""')  fmt(2)) p(fmt(2))) starlevels(* 0.10 ** 0.05 *** 0.01) append
				if `groupvar' == 3 estout * using "$tables\TableA4.xls" ,  keep(D`cohort')   label  mgroups("Economically active" 	"Paid work" 		"Formal paid work"		,  pattern(1 0 0 1 0 0 1 0 0))  stats(pwolf1 N,  labels("Romano-Wolf"))  cells(b(star fmt(2) ) se(par(`"="("' `")""')  fmt(2)) p(fmt(2))) starlevels(* 0.10 ** 0.05 *** 0.01) replace
				if `groupvar' == 4 estout * using "$tables\TableA4.xls" ,  keep(D`cohort')   label  mgroups("Informal paid work" 	"Attending school" 	"Only attending school"	,  pattern(1 0 0 1 0 0 1 0 0))  stats(pwolf1 N,  labels("Romano-Wolf"))  cells(b(star fmt(2) ) se(par(`"="("' `")""')  fmt(2)) p(fmt(2))) starlevels(* 0.10 ** 0.05 *** 0.01) append
				if `groupvar' == 5 estout * using "$tables\TableA5.xls" ,  keep(D`cohort')   label  mgroups("Economically active" 	"Paid work" 		"Formal paid work"		,  pattern(1 0 0 1 0 0 1 0 0))  stats(pwolf1 N,  labels("Romano-Wolf"))  cells(b(star fmt(2) ) se(par(`"="("' `")""')  fmt(2)) p(fmt(2))) starlevels(* 0.10 ** 0.05 *** 0.01) replace
				if `groupvar' == 6 estout * using "$tables\TableA5.xls" ,  keep(D`cohort')   label  mgroups("Informal paid work" 	"Attending school" 	"Only attending school"	,  pattern(1 0 0 1 0 0 1 0 0))  stats(pwolf1 N,  labels("Romano-Wolf"))  cells(b(star fmt(2) ) se(par(`"="("' `")""')  fmt(2)) p(fmt(2))) starlevels(* 0.10 ** 0.05 *** 0.01) append
				estimates clear
			}
			 cap erase "$inter\mht_inside_bandwidth.dta"
