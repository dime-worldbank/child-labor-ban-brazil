				...
				foreach variable in eap pwork pwork_formal pwork_informal schoolatt study_only	{
					**
					...
					replace `variable' = `variable'*100
						...
						local dep_var = `dep_var' + 1
					if "`variable'" == "eap" estout * using "$tables\Table1.xls",  keep(Conventional)   title("`title'") label  stats(Obs)  cells(b(star fmt(2) ) se(par(`"="("' `")""')  fmt(2))) starlevels(* 0.10 ** 0.05 *** 0.01) replace
					if "`variable'" != "eap" estout * using "$tables\Table1.xls",  keep(Conventional)   title("`title'") label  stats(Obs)  cells(b(star fmt(2) ) se(par(`"="("' `")""')  fmt(2))) starlevels(* 0.10 ** 0.05 *** 0.01) append
				}
	}
